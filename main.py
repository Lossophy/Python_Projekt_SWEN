...
def reise_neu_submit():
    ...
    if not name or not start or not ende:
        ...
        return render_template(
            ...
            form={"name": name, "startdatum": start, "enddatum": ende, "beschreibung": beschreibung},
        )

    r = Reise(name=name, startdatum=_parse_date(start), enddatum=_parse_date(ende), beschreibung=beschreibung)
    _reisen[_next_id] = r
    rid = _next_id
    _next_id += 1
...
def gegenstand_hinzufuegen(reise_id: int):
    ...
    if name and 0 <= kat_index < len(r.kategorien):
        r.kategorien[kat_index].gegenstand_hinzufuegen(Gegenstand(name=name, menge=menge))
    return redirect(url_for("reise_detail", reise_id=reise_id))
